...
impl State {
    fn reset(&mut self, tab_id: TabId) {
        match tab_id {
            TabId::Counter => self.counter_tab = CounterTab::new(),
            TabId::Ferris => self.ferris_tab = FerrisTab::new(),
        }
    }
}
        ...
        match self {
            ...
            TabBarExample::Loaded(state) => match message {
                Message::TabSelected(selected) => {
                    state.active_tab = selected.clone();
    ...
    fn view(&self) -> Element<'_, Self::Message> {
        match self {
            ...
            TabBarExample::Loaded(state) => {
                // let position = state.settings().tab_bar_position.unwrap_or_default();
                // let theme = state.settings().tab_bar_theme.clone().unwrap_or_default();
